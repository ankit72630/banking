...
export const getClientInfo = (): any => {
  ...
  const SSCode = getUrl.getParameterByName("SSCode"); //ibanking token
    ...
    return axios
      ...
      .then((formConfigRes) => {
        if (
          ...
          if (createResponse) {
            for (let key in createResponse.applicant) {
              if (createResponse.applicant[key]) {
                formConfigRes.data.applicants[key] =
                  createResponse.applicant[key];
              }
            }
          }
          dispatch(
...
export const redirectingToIbanking = async () => {
  if (getUrl.getParameterByName("SSCode")) {
    const baseUrl = `${process.env.REACT_APP_RTOB_BASE_URL}`;
    const endpoint = `${process.env.REACT_APP_RTOB_LOGIN_END_POINT}`;
    const options = {
      method: "GET",
      url: baseUrl + endpoint,
      headers: {
        Authorization: "ETB " + getUrl.getParameterByName("SSCode") + "&LOGOUT",
      },
    };
    await axios.request(options).then((response: any) => {
      if (response && response.data) {
        window.location.href =
          `${process.env.REACT_APP_IBANKING_URL}` + response.data;
      } else {
        window.location.href = `${process.env.REACT_APP_IBANKING_URL}`;
      }
    });
  } else {
    window.location.href = `${process.env.REACT_APP_IBANKING_URL}`;
